...
import { getMotifProfiles } from '../src'

jest.setTimeout(1000000)

describe('Users', () => {
  describe('getMotifProfiles', () => {
    const motifProfilesApiUri = 'https://motif.co/api/users'
    describe('success', () => {
      it('makes a POST request to /api/users and returns data', async () => {
        let mock = new MockAdapter(axios)
        mock.onPost(motifProfilesApiUri).reply(200, [{ foo: 'bar' }])
        const profiles = await getMotifProfiles(['test'])
    ...
    describe('input validation', () => {
      ...
      it('raises if too many addresses', async () => {
        ...
        try {
          await getMotifProfiles(addresses)
          throw new Error('should throw')
        } catch (err) {
    ...
    describe('request error handling', () => {
      ...
      it('throws with error message from response', async () => {
        mock.onPost(motifProfilesApiUri).reply(400, 'Custom error message from motif api')
        try {
          await getMotifProfiles(['asdf', '1234'])
          throw new Error('should throw')
        } catch (err) {
          expect(err).toEqual(Error('Custom error message from motif api'))
        }
      })

      it('raises if axios request fails', async () => {
        mock.onPost(motifProfilesApiUri).networkError()
        try {
          await getMotifProfiles(['asdf', '1234'])
          throw new Error('should throw')
        } catch (err) {
      ...
      it('raises if axios response has no data', async () => {
        mock.onPost(motifProfilesApiUri).reply(200, null)
        try {
          await getMotifProfiles(['asdf', '1234'])
          throw new Error('should throw')
        } catch (err) {
      ...
      it('raises if axios response data is not a list', async () => {
        mock.onPost(motifProfilesApiUri).reply(200, {})
        try {
          await getMotifProfiles(['asdf', '1234'])
          throw new Error('should throw')
        } catch (err) {
