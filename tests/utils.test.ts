...
import { Decimal, Motif } from '../src'
import { Blockchain, generatedWallets } from '@motif-foundation/asset/dist/utils'
import { JsonRpcProvider } from '@ethersproject/providers'
import { mintCurrency, setupMotif, MotifConfiguredAddresses } from './helpers'
...
describe('Utils', () => {
  ...
  describe('Type Constructors', () => {
    ...
    describe('#constructItemData', () => {
      it('creates ItemData', () => {
        const item = constructItemData(
          defaultTokenURI,
          defaultMetadataURI,
          contentHash,
          metadataHash
        )
        expect(item.tokenURI).toBe(defaultTokenURI)
        expect(item.metadataURI).toBe(defaultMetadataURI)
        expect(item.contentHash).toBe(contentHash)
        expect(item.metadataHash).toBe(metadataHash)
      })
  ...
  describe('EIP-712 Utilities', () => {
    describe('#signPermitMessage', () => {
      ...
      beforeAll(() => {
        motifConfig = {
          itemExchange: '0x1D7022f5B17d2F8B695918FB48fa1089C9f85401',
          item: '0x1dC4c1cEFEF38a777b15aA20260a54E584b16C48',
          currency: '',
        }
      })
      ...
      it('signs a permit message that is able to be processed on chain', async () => {
        const provider = new JsonRpcProvider()
        const [mainWallet, otherWallet] = generatedWallets(provider)
        const onChainConfig = await setupMotif(mainWallet, [otherWallet])
        const mainMotif = new Motif(
          mainWallet,
          50,
          onChainConfig.item,
          onChainConfig.itemExchange
        )
        ...
        await mainMotif.mint(itemData, bidShares)
    ...
    describe('#signMintWithSig', () => {
      ...
      it('signs a mintWithSig message that is able to be processed on chain', async () => {
        ...
        await otherMotif.mintWithSig(mainWallet.address, itemData, bidShares, eipSig)
  ...
  describe('Miscellaneous', () => {
    ...
    describe('isItemDataVerified', () => {
      it('returns true if the itemDatas uri match their hashes', async () => {
        const mock = new MockAdapter(axios)
        ...
        const itemData = constructItemData(kanyeURI, helloWorldURI, kanyeHash, hash)
        const verified = await isItemDataVerified(itemData)
        expect(verified).toEqual(true)
      })
    })
